...
export const useComunicationStore = defineStore("comunication", () => {
    ...
    const getDiagnosticos = async (doctorId: number, patientId: number) => {
        ...
        if(doctorId){
            const res = await DoAxiosWithErro(`/medical/doctor/${doctorId}/diagnoses`,'get',{},true)
            digList.splice(0, digList.length);
            digList.push(...res)
            return;
        }
        if(patientId){
            const res = await DoAxiosWithErro(`/medical/patient/${patientId}/diagnoses`,'get',{},true)
            digList.splice(0, digList.length);
            digList.push(...res)
        }
    }
    ...
    const getHistory = async (diagId: number) => {
       return await DoAxiosWithErro(`/medical/diagnoses/${diagId}/feedback`, 'get', {}, true)
    }
